        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    ...
                    String role = email.equals("admin@butorbolt.hu") ? "admin" : "user";
                    String uid = mAuth.getCurrentUser().getUid();

                    FirebaseFirestore.getInstance().collection("users").document(uid)
                            .set(new UserData(userName, email, phoneNumber, address, role))
                            .addOnSuccessListener(unused -> {
                                Log.d(LOG_TAG, "Felhasználó Firestore-ba mentve");
                                openShop();
                            })
                            .addOnFailureListener(e -> {
                                Log.e(LOG_TAG, "Nem sikerült menteni Firestore-ba", e);
                                Toast.makeText(RegisterActivity.this, "Nem sikerült menteni az adatokat", Toast.LENGTH_SHORT).show();
                            });
                }else{
